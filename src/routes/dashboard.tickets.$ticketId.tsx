...
function TicketDetail() {
  ...
  const t = useStore(s => s.tickets.find(x => x.id === ticketId));
  if (!t) {
    return (
      <div className="text-center py-12">
        <div className="font-semibold">Ticket not found</div>
        <Link to="/dashboard/tickets" className="text-neon-cyan text-sm mt-2 inline-block">← Back to tickets</Link>
      </div>
    );
  }
    ...
    <div className="space-y-6 max-w-5xl animate-fade-in">
      ...
      <div className="grid lg:grid-cols-[1fr_300px] gap-6">
        <div className="glass rounded-2xl p-6 border border-neon-blue/20">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold">{t.eventName}</h2>
          ...
          <div className="mt-6 rounded-xl bg-background/40 border border-border p-4 space-y-3 text-xs font-mono">
            <div className="flex items-start justify-between gap-3">
              <span className="text-muted-foreground flex items-center gap-1.5 shrink-0"><FileCode2 className="h-3.5 w-3.5" />Contract</span>
              <span className="text-neon-cyan break-all text-right">{t.contractAddress}</span>
            </div>
            <div className="flex items-start justify-between gap-3">
              <span className="text-muted-foreground flex items-center gap-1.5 shrink-0"><ShieldCheck className="h-3.5 w-3.5" />Tx Hash</span>
              <span className="text-neon-purple break-all text-right">{t.txHash}</span>
            </div>
            <div className="flex items-start justify-between gap-3">
              <span className="text-muted-foreground flex items-center gap-1.5 shrink-0"><Database className="h-3.5 w-3.5" />IPFS CID</span>
              <span className="text-neon-cyan break-all text-right">{t.ipfsCid}</span>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="glass rounded-2xl p-5 border border-neon-cyan/30 text-center animate-scale-in">
            ...
            <div className="bg-foreground p-3 rounded-xl inline-block">
              <QRCodeSVG value={t.id} size={180} bgColor="#fff" fgColor="#000" />
            </div>
          ...
          <div className="rounded-xl border border-border bg-card/50 p-4 text-xs space-y-2">
            <div className={`flex items-center gap-2 ${t.status === "Active" ? "text-success" : t.status === "Used" ? "text-warning" : "text-destructive"}`}>
              <span className={`h-2 w-2 rounded-full ${t.status === "Active" ? "bg-success animate-pulse" : t.status === "Used" ? "bg-warning" : "bg-destructive"}`} />
              {t.status === "Active" ? "On-chain ownership confirmed" : t.status === "Used" ? "Already redeemed at gate" : "Invalid"}
            </div>
